    ...
    for root, dirs, files in os.walk(mailpath):
        for name in files:
            ...
            try:
                ...
                mailToList = [EmailTo(email_id = email.msgId,
                                        sender = email.fromAddress,
                                        to_address = receiver_to) for receiver_to in enumerate(email.toAddress)]
                EmailTo.objects.bulk_create(mailToList)



                mailCcList = [EmailCc(email_id=email.msgId,
                                      sender=email.fromAddress,
                                      cc_address=receiver_cc) for receiver_cc in enumerate(email.ccAddress)]
                EmailCc.objects.bulk_create(mailCcList)


                mailbccList = [EmailBcc(email_id=email.msgId,
                                      sender=email.fromAddress,
                                      bcc_address=receiver_bcc) for receiver_bcc in enumerate(email.bccAddress)]
                EmailBcc.objects.bulk_create(mailbccList)
